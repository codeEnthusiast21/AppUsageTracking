...
class TimedBlockingActivity : AppCompatActivity() {
    ...
    private fun updateText(newText: String) {
        val slide: Animation = AnimationUtils.loadAnimation(this, R.anim.slider)
        binding.tvQuotes.text = newText
        binding.tvQuotes.startAnimation(slide) // Restart animation after updating text
    }
    ...
    private fun fetchQuote() {
        RetrofitClient.qouteApi.getRandomQuote().enqueue(object: retrofit2.Callback<List<QuotesModel>> {
                ...
                override fun onFailure(call: Call<List<QuotesModel>>, t: Throwable) {
                    updateText( "Failed to load data")
                }
            },
        )
}
